  ...
  namespace algo {
	...
	if(length>2)
      ...
      void grid(GRAPH& g, 
		...
		const VERTEX_INIT& vinit,
		const EDGE_INIT& einit) {
	std::vector< std::vector<typename GRAPH::ref_vertex_type> > grid;
	unsigned int w,h;

	grid.resize(height);
	for(h=0; h<height; ++h) {
	  std::vector<typename GRAPH::ref_vertex_type>& row(grid[h]);
	  row.resize(width);
	  for(w=0; w<width; ++w) 
	    grid[h][w] = (g += vinit(w,h));
	}

	for(h=0; h<height-1; ++h)
	  for(w=0; w<width-1; ++w) {
	    g.connect(einit(w,h,w,h+1),grid[h][w],grid[h+1][w]);
	    g.connect(einit(w,h,w+1,h),grid[h][w],grid[h][w+1]);
	  }

	if(height > 0)
	  for(w=0; w<width-1; ++w)
	    g.connect(einit(w,height-1,w+1,height-1),grid[height-1][w],grid[height-1][w+1]);
	if(width > 0)
	  for(h=0; h<height-1; ++h)
	    g.connect(einit(width-1,h,width-1,h+1),grid[h][width-1],grid[h+1][width-1]);
      }
    }
