...
namespace vq2 {
  namespace unit {
    ...
    public:
       ...
       */
      UnitToSampleSimilarity(SIMILARITY& similarity) : sim(similarity) {}
      /**
    ...
    /**
     * This implements a similarity between two units and a sample using
     * the unit prototypes.
     */
    template<typename UNIT,typename SIMILARITY>
    class UnitToUnitSimilarity {
    private:
      SIMILARITY& sim;

    public:
      typedef UNIT value_type;
      typedef UNIT sample_type;

      /**
       * @param similarity compares prototypes to samples.
       */
      UnitToUnitSimilarity(SIMILARITY& similarity) : sim(similarity) {}
      /**
       * @param arg1 This is what the vertices store (vertex.value)
       * @param arg2 This is some external data.
       * @return a <b>positive</b> value that is close to zero if arg1 and arg2 are close.
      ...
      double operator()(const value_type& arg1,
			const sample_type& arg2) {
	return sim(arg1.prototype(),arg2.prototype());
      }
    };
